...
export class PlaylistService {
  ...
  async create(data: any, image?: Express.Multer.File) {
    ...
    if (image) {
      if (image.mimetype.startsWith('image/'))
        data.cover_image = `/uploads/images/${image.filename}`;
  ...
  async update(id: number, data: any, image?: Express.Multer.File) {
    ...
    if (!user) throw new NotFoundException('User not found');
    const playlist = await this.playlistRepository.findOne({ where: { id, users: { id: user_Id } }});
  ...
  async syncTrack(userId: number, id: number, TrackIds: number[]) {
  ...
  async addTrack(userId: number, id: number, TrackIds: number[]) {
    const user = await this.userRepository.findOne({ where: { id: userId } });
    if (!user) throw new NotFoundException('User not found');
    const playlist = await this.playlistRepository.findOne({
      where: { id, users: { id: userId } },
  ...
  async deleteTracks(userId: number,id: number) {
    const user = await this.userRepository.findOne({ where: { id: userId } });
    if (!user) throw new NotFoundException('User not found');
    const playlist = await this.playlistRepository.findOne({
      where: { id , users: { id: userId } },
